...
export function computeRelayFeeUsd(
  ...
  const relayFeeEthWei = routeResult.txReq?.value
    ? BigInt(routeResult.txReq.value)
    : 0n;
  if (relayFeeEthWei === 0n) return 0;

  const feeCosts = (routeResult.route?.estimate?.fees ?? []) as Array<
  ...
  if (usd === 0) {
    const fromUsd = parseFloat(
      routeResult.route?.estimate?.fromAmountUsd ?? "0"
    );
    const toMinUsd = parseFloat(
      routeResult.route?.estimate?.toAmountMinUsd ?? "0"
    );
...
export function computeAdjustedSliderMax(
  ...
  const reserve =
    tokenPriceUSD > 0 && relayFeeUsd > 0
      ? (relayFeeUsd / tokenPriceUSD) * 1.15
      : 0;
